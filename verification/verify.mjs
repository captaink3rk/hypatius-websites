/* verification/verify.mjs · hypati.us v3 page harness
   Cloned from the STARCHITECT v9 harness; same checks, hypati.us pages and Blackout palette.
...
const SCREENS=[
  ...
  ['platforms', '../hypatius-website/platforms.html'],
  ['company',   '../hypatius-website/company.html'],
  ['news',      '../hypatius-website/news.html'],
  ['release',   '../hypatius-website/news-stanley-kennedy-ceo.html'],
  ['gate',      '../hypatius-website/index.html'],
];
...
for (const [s,path,only] of SCREENS){
  for (const [w,h] of [[1440,900],[390,844]].filter(v=>!only||only.includes(v[0]))){
    ...
    // focus ring
    await p.keyboard.press('Tab'); if(await p.evaluate(()=>document.activeElement?.classList.contains('skip'))) await p.keyboard.press('Tab'); const fr=await p.evaluate(()=>{ const e=document.activeElement; if(!e||e===document.body) return 'none'; const cs=getComputedStyle(e); return (cs.outlineStyle!=='none'&&parseFloat(cs.outlineWidth)>0)?'ok':`no outline on ${e.tagName}.${e.className.toString().split(' ')[0]}`; });
