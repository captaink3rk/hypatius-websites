/* verification/verify-site.mjs · starchitect.us v10 website harness
   Cloned back from the hypati.us harness, which was itself cloned from the
   STARCHITECT *platform* harness. That platform harness covers the fourteen
   operator-console screens and has never covered the website, so until now the
   nine public pages of starchitect.us had never been verified at all.
   ...
   Checks every v2 surface at 1440 and 390:
     ...
     8. banned terms in visible text (HYPATIUS house style permits em dashes; that check is off here)
   Exit code 1 on any failure. Report written to verification/report-site.json. */
...
const SCREENS=[
  ['home',         '../starchitect-website/home.html'],
  ['platform',     '../starchitect-website/platform.html'],
  ['capabilities', '../starchitect-website/capabilities.html'],
  ['use-cases',    '../starchitect-website/use-cases.html'],
  ['resilience',   '../starchitect-website/resilience.html'],
  ['security',     '../starchitect-website/security.html'],
  ['acquisition',  '../starchitect-website/acquisition.html'],
  ['company',      '../starchitect-website/company.html'],
  ['resources',    '../starchitect-website/resources.html'],
  ['press-kit',    '../starchitect-website/press-kit.html'],
];
...
for (const [s,path,only] of SCREENS){
  for (const [w,h] of [[1440,900],[390,844]].filter(v=>!only||only.includes(v[0]))){
    ...
    const [file,hash]=path.split('#'); await p.goto('file://'+resolve(here,file)+(hash?'#'+hash:''),{waitUntil:'load'}); await p.waitForTimeout(500); await p.addStyleTag({content:'.reveal{opacity:1!important;transform:none!important}'}); await p.waitForTimeout(100);
    const r=await p.evaluate(()=>{
      ...
      let n; while((n=walker.nextNode())){ const t=n.textContent.trim(); if(!t) continue; const el=n.parentElement; if(!el||!el.offsetParent&&el.tagName!=='BODY') continue; const cs=getComputedStyle(el); if(cs.display==='none'||cs.visibility==='hidden') continue; const fs=parseFloat(cs.fontSize); txt.push(t); if(fs<10 && !el.closest('svg') && !el.closest('.mdofig__frame')) small.push(`${fs}px "${t.slice(0,30)}"`); }
      ...
      inter.forEach(e=>{ const r=e.getBoundingClientRect(); if(r.width<44&&r.height<44&&!e.classList.contains('skip')) tiny.push(`${e.tagName}.${e.className.toString().split(' ')[0]} ${Math.round(r.width)}x${Math.round(r.height)}`); });
      ...
      const bgOf=el=>{ // composite every translucent layer from the element up to an opaque one
        ...
        let out=[4,9,15]; for(const l of layers.reverse()){ out=out.map((v,i)=>Math.round(l[i]*l[3]+v*(1-l[3]))); } return `rgb(${out.join(', ')})`; };
    ...
    // focus ring
    await p.keyboard.press('Tab'); if(await p.evaluate(()=>document.activeElement?.classList.contains('skip'))) await p.keyboard.press('Tab');
    /* Wait a frame before measuring. Reading getComputedStyle immediately after
       a synthetic Tab returns a STALE outline-width of 0 in this headless
       build: the focus rule has matched and :focus is live, but the style
       recalculation has not flushed. That produced a confident, wrong
       "no outline" on all nine pages while the ring was in fact rendering.
       Anything that forces a recalc fixes it; a rAF is the honest way. */
    await p.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));
    const fr=await p.evaluate(()=>{ const e=document.activeElement; if(!e||e===document.body) return 'none'; const cs=getComputedStyle(e); return (cs.outlineStyle!=='none'&&parseFloat(cs.outlineWidth)>0)?'ok':`no outline on ${e.tagName}.${e.className.toString().split(' ')[0]}`; });
